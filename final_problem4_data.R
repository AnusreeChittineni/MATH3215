...
# run this loop first!
for (d in 1:10) {
  model <- lm(Y ~ poly(x, d, raw = TRUE))  # Fit polynomial regression
  print(summary(model)$coefficients)      # Inspect coefficients
}

d_final <- 4 # put value that results from above loop
final_model <- lm(Y ~ poly(x, d_final, raw = TRUE))

# Extract p-value and coefficient information
summary(final_model)  

alpha1 <- coef(final_model)[2]  
se_alpha1 <- summary(final_model)$coefficients[2, 2]  
df <- length(x) - d_final - 1
t_value <- qt(0.975, df = length(x) - d_final - 1)  
ci_lower <- alpha1 - t_value * se_alpha1
ci_upper <- alpha1 + t_value * se_alpha1
c(ci_lower, ci_upper)

cat("Alpha1:", alpha1, "\n")
cat("95% Confidence Interval for Alpha1:", ci_lower, "to", ci_upper, "\n")

p_value <- 2 * pt(-abs(alpha1 / se_alpha1), df)
cat("P-value for H0 (alpha1 = 0):", p_value, "\n")
